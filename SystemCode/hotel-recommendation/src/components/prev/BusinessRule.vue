<template>
  ...
  export default {
    props: ['hotel', 'numberOfPeople', 'checkInDate', 'checkOutDate'], // 从路由传递的参数
    data() {
      return {
        price: null, // 存储从后端businessrule算出来的价格
      };
    },
    mounted() {
      if (this.hotel && this.hotel.id) {
        this.fetchPrice(); // 获取价格信息
      } else {
    ...
    methods: {
      async fetchPrice() {
        try {
          ...
          this.price = response.data.price; // 从后端获取价格
        } catch (error) {
